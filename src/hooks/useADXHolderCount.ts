...
export default function useADXHolderCount(): number | null {
  ...
  const loadHolders = useCallback(async () => {
    ...
    try {
      ...
      const directHolderNumber = accounts.map((account) => {
          const data = AccountLayout.decode(new Uint8Array(account.account.data));
      ...
      .filter((holder) => {
        const amountInUI = holder.amount / Math.pow(10, window.adrena.client.adxToken.decimals);
        return amountInUI > 0;
      }).length;

      const nbStakedHolders = (allStaking || []).filter((staking) => staking.stakingType === 1 && (staking.liquidStake.amount.toNumber() > 10 || staking.lockedStakes.length > 0)).length;
