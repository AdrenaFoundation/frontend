...
export default function usePositionStats(
  isByWalletAddress = false,
  viewOnlyWalletAddress: string | null = null,
) {
  const [data, setData] = useState<GetPositionStatsReturnType<{
    showPositionActivity: true;
  }> | null>(null);

  const activeUserWalletAddress = useSelector(
    (state) => state.walletState.wallet,
  )?.walletAddress;

  const walletAddress = viewOnlyWalletAddress ?? activeUserWalletAddress;
    ...
    const maxTotal = (key: keyof typeof activity) =>
      Math.max(
        ...activity.map((activity) =>
          Math.abs(activity[key as keyof typeof activity] as number),
        ),
      );
