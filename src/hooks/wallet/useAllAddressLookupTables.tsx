...
export function useAllAddressLookupTables({
    ...
    allAddressLookupTables: { pubkey: PublicKey; account: AddressLookupTableState }[] | null;
    triggerAllAddressLookupTablesReload: () => void;
} {
    const [trickReload, triggerReload] = useState<number>(0);
    const [allAddressLookupTables, setAllAddressLookupTables] = useState<{ pubkey: PublicKey; account: AddressLookupTableState }[] | null>(null);
    ...
    useEffect(() => {
        const loadAllAddressLookupTables = async () => {
            ...
            try {
                setAllAddressLookupTables(await window.adrena.client.getAllLookupTablesByAuthority(wallet));
            } catch (e) {
