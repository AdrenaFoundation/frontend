...
export function useAllUserSuperchargedProfiles({
  ...
  const [tradersProfiles, setTradersProfiles] = useState<
    TraderProfileInfo[] | null
  >(null);
    ...
    superchargedUserProfiles: useMemo(() => {
      ...
      const superchargedProfiles: {
        ...
        tradersProfiles?.map((traderProfile) => {
          const profile = allUserProfiles?.find((profile) =>
            traderProfile.userPubkey.equals(profile.owner),
          );
      ...
      allUserProfiles?.reverse().forEach((profile) => {
        if (
          superchargedProfiles?.find((superchargedProfile) =>
            superchargedProfile.wallet.equals(profile.owner),
          )
        ) {
          return;
        }
