...
export default function Referral({
    ...
    const handleViewChange = (newView: 'asReferrer' | 'asReferee') => {
        ...
        router.push(`/referral?view=${newView === 'asReferrer' ? 'referrer' : 'referee'}`, undefined, { shallow: true });
    };
                    ...
                    <div className="relative flex flex-col items-center w-full h-[17em] pt-12 border-b">
                        ...
                        <div className="z-10 text-center flex flex-col items-center justify-center gap-4">
                            <div className="flex gap-2">
                                <h4
                                    className={twMerge('font-archivo text-white/80 tracking-widest uppercase text-md cursor-pointer hover:opacity-100', view === 'asReferrer' ? 'underline opacity-100' : 'opacity-50')}
                                ...
                                <h4>/</h4>
                                <h4
                                    className={twMerge('font-archivo text-white/80 tracking-widest uppercase text-md cursor-pointer hover:opacity-100', view === 'asReferee' ? 'underline opacity-100' : 'opacity-50')}
                            ...
                            >
                                {view === 'asReferrer' ? 'SPREAD THE WORD' : 'SHOW YOUR SUPPORT'}
                            </h1>

                            <h4 className='font-archivo text-white/80 tracking-widest uppercase text-md'>{view === "asReferrer" ? 'And receive 10% of the trading fee' : 'And grant 10% of your trading fees'}</h4>
                        </div>
                    </div>

                    <div className='flex flex-col relative w-full'>
                        {userProfile === false && connected ? <div
                        ...
                        {!connected ? <div
                            className='absolute w-full flex items-center justify-center top-[calc(50%-1em)] z-10 opacity-60'
                        >
                            Connect to access the page
                        </div> : null}

                        {view === 'asReferrer' ?
                            <Referrer userProfile={userProfile} setActiveProfile={setActiveProfile} connected={connected} /> :
                            <Referee userProfile={userProfile} setActiveProfile={setActiveProfile} connected={connected} />}
                    </div>
                </StyledContainer>
            </div>
