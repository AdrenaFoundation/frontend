...
export default function AddressLookupTable({}: { view: string }) {
  const { allAddressLookupTables, triggerAllAddressLookupTablesReload } =
    useAllAddressLookupTables({
  ...
  const addressLookupTableDOM = useMemo(() => {
    ...
    return allAddressLookupTables.map(({ pubkey, account }) => {
      return (
        <div
          key={pubkey.toBase58()}
          className="text-sm border p-4 w-full flex flex-col items-center gap-2"
        >
          ...
          {account.addresses.length
            ? account.addresses.map((address, i) => (
                <div key={address.toBase58()} className="flex w-full">
          ...
          {additionalAccounts.accounts[pubkey.toBase58()]?.map((address, i) => {
            return (
              <InputString
                ...
                value={address}
                className={twMerge(
                  'pt-[0.5em] pb-[0.5em] pl-4 pr-4 border border-gray-700 bg-transparent rounded-md w-[90%] text-txtfade placeholder:text-txtfade',
                )}
                inputFontSize="0.8em"
              />
            );
            ...
            onClick={async () => {
              ...
              try {
                await window.adrena.client.extendLookupTable({
                  ...
                  notification,
                  addresses:
                    additionalAccounts.accounts[pubkey.toBase58()]?.map(
  ...
  return (
    <StyledContainer
      className="p-4 w-full relative items-center flex"
      bodyClassName="gap-1 flex items-center w-full"
    >
      <div
