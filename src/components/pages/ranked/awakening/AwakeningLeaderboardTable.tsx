    ...
    return (
        <div className={className}>
            ...
            <div className="flex flex-row items-center gap-3 mt-3">
                <h3 className={twMerge("font-medium capitalize", division === 'No Division' ? 'ml-auto mr-auto' : '')}>{DIVISIONS[division].title}</h3>
                ...
                <Tippy content={`Top ${DIVISIONS[division].topTradersPercentage} percentile of traders by traded VOLUME, minus the ones on previous divisions.`} arrow>
                    <div className={twMerge(`capitalize text-sm tracking-widest font-medium ${division === 'No Division' ? 'hidden' : ''}`, DIVISIONS[division].color)}>
                        TIER {index}
                        <div className={`border-b-2 border-dotted border-gray-400 mt-0`}></div>
                    </div>
                </Tippy>

                {myDivision ? <div className='font-medium text-xs bg-yellow-900 bg-opacity-40 rounded-lg border border-yellow-900 pt-1 pr-2 pl-2 pb-1 w-20 text-center'>Your division</div> : null}
            </div>
                            ...
                            values: [
                                ...
                                d.username
                                    ? isValidPublicKey(d.username)
                                        ? <p key={`trader-${i}`} className={twMerge('text-xs font-medium opacity-50', d.connected ? 'text-yellow-600' : '')}>{getAbbrevWalletAddress(d.username)}</p>
                                        : <p key={`trader-${i}`} className={twMerge('text-xs font-medium hover:underline transition duration-300 cursor-pointer', d.connected ? 'text-yellow-600 ' : '')} onClick={() => handleProfileView(d.username)}>
                                            {d.username.length > 16 ? `${d.username.substring(0, 16)}...` : d.username}
                                        </p>
                                    : <p key={`trader-${i}`} className='text-xs font-medium'>-</p>
                                ,
                                ...
                                <div className='flex items-center justify-end md:justify-center grow' key={`pnl-${i}`}>
                                    <FormatNumber
                                        nb={d.pnl}
                                        format="currency"
                                        className={twMerge('text-xs font-medium', d.pnl && (d.pnl >= 0) ? 'text-green' : 'text-red')}
                                ...
                                <div className='flex flex-col items-end ml-auto' key={`rewards-${i}`}>
                                    {d.adxRewards ? <div className='flex'>
                                        ...
                                        <span className='flex text-green font-medium text-xs ml-1'>ADX</span>
                                    </div> : null}
                                    ...
                                    {d.adxRewards ? <div className='flex'>
                                        <FormatNumber
                                            ...
                                            isDecimalDimmed={false}
                                        />
                                        <span className='flex text-green font-medium text-xs ml-1'>JTO</span>
                                    </div> : null}
