...
const AwardHeader = memo(
  ({ title, type }: { title: string; type: 'reward' | 'ticket' }) => (
    <div className="flex flex-col items-center gap-2">
      <Image
        src={type === 'ticket' ? diceImage : firstImage}
        alt="first place logo"
        className={twMerge('h-10', type === 'ticket' ? 'opacity-80' : '')}
        width={40}
        height={40}
      />
    ...
    <div className="flex items-center justify-center h-[3em]">
      {trader === null || trader === '' ? (
        <>
          ...
          <Image src={ticketImage} alt="ticket image" className="w-10 h-8" />
        </>
      ) : (
