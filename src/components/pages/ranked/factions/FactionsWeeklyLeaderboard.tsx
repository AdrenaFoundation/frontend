...
export default function FactionsWeeklyLeaderboard({
  ...
  const calculateRewardValue = (rewardTokens: {
    ADX?: number;
    BONK?: number;
    JTO?: number;
  }) => {
  ...
  const dataReady = useMemo(() => {
    ...
    return data.map((d, i) => {
      const title = Object.entries(officers).find(
        ([, v]) => v.wallet.toBase58() === d.userWallet,
      )?.[0];
      ...
      return {
        ...
        specificRowClassName: twMerge(
          wallet?.walletAddress === d.userWallet
  ...
  return (
    <div className={twMerge('w-full max-w-[35em] flex flex-col items-center')}>
      <div className="flex flex-col items-center justify-center mb-8">
        <div className="font-archivo tracking-[0.3em] uppercase">
          {team === 'A'
