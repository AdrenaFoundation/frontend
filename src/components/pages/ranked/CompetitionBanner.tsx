...
export default function CompetitionBanner({
  ...
  const totalPrize = useMemo(() => {
    return (
      adxRewards * (tokenPrices.ADX ?? 0) +
      jtoRewards * (jtoPrice ?? 0) +
      bonkRewards * (tokenPrices.BONK ?? 0) +
      usdcRewards
    );
  }, [
    adxRewards,
    ...
    return (
        ...
        {adxRewards + jtoRewards + bonkRewards + usdcRewards > 0 ? (
          <div className="flex flex-col sm:mt-8 z-10 items-center">
            ...
            <div
              className={twMerge(
                'w-[20em] max-w-full flex items-center justify-center rounded-md flex-col',
              )}
            >
              {seasonName === 'factions' ? (
                <FormatNumber
        ...
        {seasonName !== 'anniversary1' ? (
          <div
            className={twMerge(
              'flex flex-row items-center gap-3 z-10 absolute bottom-16 sm:bottom-6 sm:right-8',
              startDate && startDate.getTime() >= Date.now()
                ? 'mt-[2em]'
                : 'mt-[4em]',
            )}
