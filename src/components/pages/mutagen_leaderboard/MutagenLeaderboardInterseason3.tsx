...
type SortField = 'totalVolume' | 'pointsTrading' | 'rewards';
...
const calculateRewards = (rank: number) => {
  ...
  const rewardsDistribution = [
    ...
    { jto: 0.1, bonk: 0.1 }, // 3rd place: 10% of each
...
export default function MutagenLeaderboardInterseason3({
  ...
  const [userRow, setUserRow] = useState<MutagenLeaderboardData[number] | null>(
    null,
  );
  const [sortedTraders, setSortedTraders] =
    useState<MutagenLeaderboardData | null>(null);
  const wallet = useSelector((s) => s.walletState.wallet);
  const tokenPrices = useSelector((s) => s.tokenPrices);

  // Calculate total reward value in USD
  const calculateRewardValue = useCallback(
    (jtoRewards: number, bonkRewards: number) => {
  ...
  const renderProgressMessage = useCallback(
    (isMobile: boolean = false) => {
      ...
      const messageClass =
        'text-sm font-archivoblack animate-text-shimmer bg-clip-text text-transparent bg-[length:250%_100%] bg-[linear-gradient(110deg,#FA6724,45%,#FAD524,55%,#FA6724)]';

      if (userRow.rank === 1) {
        return <span className={messageClass}>You are the champion!</span>;
      }

      const personAbove = sortedTraders.find(
        (d) => d.rank === userRow.rank - 1,
      );
  ...
  const renderCardContainer = useCallback(
    (children: React.ReactNode, isMobile: boolean = false) => {
      if (!userRow) return null;
  ...
  const mobileMutagenCardLayout = useMemo(() => {
    ...
    return renderCardContainer(
      <div className="flex flex-col items-center justify-center">
        ...
        <div className="flex items-center gap-1 mt-2">
          <span className="font-boldy text-white text-sm">
            Mutagen (Trading):
          </span>
        ...
        <div className="text-center mb-2">{renderProgressMessage(true)}</div>
      </div>,
      true,
    );
  }, [
        ...
        <div className="flex flex-row items-center w-full">
          <div className="flex flex-row items-center gap-3 flex-1 justify-center">
            ...
            <div className="flex flex-col items-start text-left">
              <span className="font-boldy text-white text-lg sm:text-xl truncate">
                {userRow.nickname}
              </span>
              {userRow.title !== null && (
        ...
        <div className="flex w-full flex-row items-center gap-8 justify-center mt-2 mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-boldy">
              <span
                className={totalRewardUsd > 0 ? 'text-green' : 'text-white'}
              >
                {formatPriceInfo(totalRewardUsd, 0, 0)}
              </span>
            </span>
          ...
          <div className="flex">{renderProgressMessage(false)}</div>
        </div>
      </div>,
    );
  }, [
  ...
  useEffect(() => {
    ...
    const sortFunction = (
      a: MutagenLeaderboardData[number],
      b: MutagenLeaderboardData[number],
    ) => {
      ...
      if (sortField === 'rewards') {
        ...
        const valueA = calculateRewardValue(
          rewardsA.jtoRewards,
          rewardsA.bonkRewards,
        );
        const valueB = calculateRewardValue(
          rewardsB.jtoRewards,
          rewardsB.bonkRewards,
        );
    ...
    return sortedTraders.map((d, i) => {
      const values = [
        ...
        >
          ...
          <div
            id={`user-mutagen-${d.userWallet.toBase58()}`}
            className={twMerge(
              'flex min-w-0 flex-1',
              d.nickname || d.profilePicture !== null
              ...
              <>
                {d.nickname ? (
                  ...
                  <p
                    key={`trader-${i}`}
                    className={twMerge(
                      'text-xs font-boldy hover:underline transition duration-300 cursor-pointer text-txtfade truncate text-left',
                    )}
                    onClick={() => {
                      onClickUserProfile(d.userWallet);
                    }}
      ...
      const totalRewardUsd = calculateRewardValue(
        rewards.jtoRewards,
        rewards.bonkRewards,
      );
    ...
    if (breakpoint5) {
      columns.push(
        <button
          ...
          key="pointsTrading"
        >
          <span className={getSortTextClass('pointsTrading')}>
            Mutagen (Trading)
          </span>
  ...
  return (
    <div className={twMerge('w-full ml-auto mr-auto mt-2', className)}>
      {userRow
        ? breakpoint3
          ? desktopMutagenCardLayout
          : mobileMutagenCardLayout
        : null}
