...
export default function MutagenLeaderboardInterseason3({
    ...
    const renderProgressMessage = useCallback((isMobile: boolean = false) => {
        ...
        const messageClass = "text-sm font-black animate-text-shimmer bg-clip-text text-transparent bg-[length:250%_100%] bg-[linear-gradient(110deg,#FA6724,45%,#FAD524,55%,#FA6724)]";
        ...
        return renderCardContainer(
            <div className="flex flex-col items-center justify-center">
                ...
                <div className="flex flex-col items-center">
                    <span className="font-medium text-white text-xl sm:text-2xl truncate text-center">
                        {userRow.nickname}
                    </span>

                    {userRow.title !== null && (
                        <span className="text-sm font-medium text-txtfade opacity-70 truncate text-center">
                ...
                <div className="flex items-center gap-1 mt-2">
                    <span className="font-medium text-white text-sm">Rank:</span>
                    <span className="text-base font-medium text-white bg-mutagen/40 px-3 py-1 rounded-full shadow">
                        #{userRow.rank}
                    </span>
                </div>

                <div className="flex items-center gap-1 mt-2">
                    <span className="font-medium text-white text-sm">Mutagen (Trading):</span>
                    <span className="text-sm font-medium text-mutagen">
                        {formatNumber(userRow.pointsTrading, 2, 0)}
                    </span>
                </div>

                <div className="flex items-center gap-1 mt-2">
                    <span className="font-medium text-white text-sm">Rewards:</span>
                ...
                <div className="flex flex-row items-center w-full">
                    <div className="flex flex-row items-center gap-3 flex-1 justify-center">
                        ...
                        <div className="flex flex-col items-start text-left">
                            <span className="font-medium text-white text-lg sm:text-xl truncate">{userRow.nickname}</span>
                            {userRow.title !== null && (
                                <span className="text-xs font-medium text-txtfade opacity-70 truncate">
                                    &quot;{USER_PROFILE_TITLES[userRow.title]}&quot;
                                </span>
                            )}
                        </div>
                        <span className="text-base font-medium text-white bg-mutagen/40 px-3 py-1 rounded-full shadow">#{userRow.rank}</span>
                    </div>
                </div>
                <div className="flex w-full flex-row items-center gap-8 justify-center mt-2 mb-2">
                    <div className="flex items-center gap-2">
                        <span className="font-medium text-white text-sm">Mutagen (Trading):</span>
                        <span className="text-sm font-medium text-mutagen">{formatNumber(userRow.pointsTrading, 2, 0)}</span>
                    </div>
                            ...
                            <>
                                {d.nickname ? (
                                    <p
                                        key={`trader-${i}`}
                                        className={twMerge(
                                            'text-xs font-medium hover:underline transition duration-300 cursor-pointer truncate text-left',
                                        )}
                                        onClick={() => {
                                    ...
                                    <p
                                        key={`trader-${i}`}
                                        className={twMerge('text-xs font-medium hover:underline transition duration-300 cursor-pointer text-txtfade truncate text-left')}
                                        onClick={() => {
                                            onClickUserProfile(d.userWallet);
                                        }}
                                    >
                                ...
                                {d.title !== null ? (
                                    <div className="text-[0.68em] font-medium text-nowrap text-txtfade truncate text-left">
            ...
            if (breakpoint5) {
                values.push(
                    ...
                    >
                        <FormatNumber
                            nb={d.pointsTrading}
                            className="text-xs font-medium text-[#e47dbb]"
                ...
                values.push(
                    ...
                    >
                        <div className="flex flex-col items-center justify-center ml-auto mr-auto">
                            <FormatNumber
                                nb={totalRewardUsd}
                                className="text-xs font-medium text-[#24af54]"
                                format='currency'
                                prefix='$'
                                isDecimalDimmed={false}
                                isAbbreviate={true}
