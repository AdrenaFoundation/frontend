...
export default function MutagenLeaderboardInterseason3({
  ...
  const renderProgressMessage = useCallback(
    (isMobile: boolean = false) => {
      ...
      const messageClass =
        'text-sm font-bold animate-text-shimmer bg-clip-text text-transparent bg-[length:250%_100%] bg-[linear-gradient(110deg,#FA6724,45%,#FAD524,55%,#FA6724)]';
        ...
        <div className="flex flex-row items-center w-full">
          <div className="flex flex-row items-center gap-3 flex-1 justify-center">
            ...
            <div className="flex flex-col items-start text-left">
              <span className="font-semibold text-white text-lg sm:text-xl truncate">
                {userRow.nickname}
              </span>
              {userRow.title !== null && (
              ...
              <>
                {d.nickname ? (
                  ...
                  <p
                    key={`trader-${i}`}
                    className={twMerge(
                      'text-xs font-semibold hover:underline transition duration-300 cursor-pointer text-txtfade truncate text-left',
                    )}
                    onClick={() => {
                      onClickUserProfile(d.userWallet);
                    }}
