          ...
          <div id={`user-mutagen-${d.userWallet.toBase58()}`}>
            {d.nickname ? (
              ...
              <p
                key={`trader-${i}`}
                className={twMerge(
                  'text-xs font-medium hover:underline transition duration-300 cursor-pointer text-txtfade',
                )}
                onClick={() => {
                  onClickUserProfile(d.userWallet);
                }}
            ...
            {d.title !== null ? (
              <div className="text-[0.68em] font-medium text-txtfade truncate max-w-[6rem]">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-3">
                <span className="font-medium text-white text-xl truncate flex-1 min-w-0">
                  {userRow.nickname}
                </span>
                {userRow.title !== null && (
                  <span className="text-xs font-medium text-txtfade md:hidden lg:flex hidden flex-shrink-0 whitespace-normal leading-tight">
                    &quot;{USER_PROFILE_TITLES[userRow.title]}&quot;
                  </span>
                )}
                <span className="font-medium text-txtfade text-xl flex-shrink-0">
                  |
                </span>
                <span className="font-medium text-white text-xl sm:flex hidden flex-shrink-0">
                  Rank:
                </span>
                <span className="text-sm font-medium text-white bg-mutagen/40 px-3 py-1 rounded-full shadow flex-shrink-0">
                  #{userRow.rank}
                </span>
                <span className="font-medium text-txtfade text-xl flex-shrink-0">
                  |
                ...
                <span className="text-xl font-medium text-mutagen sm:flex hidden flex-shrink-0">
                  {formatNumber(userRow.totalPoints, 2, 0)}
                </span>
                <span className="text-xl font-medium text-mutagen sm:hidden flex items-center flex-shrink-0">
                  <img
