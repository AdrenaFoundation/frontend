      ...
      <FormatNumber
        nb={
          ((showAfterFees ? position.pnl : position.pnl - fees) /
            ('entryCollateralAmount' in position
              ? position.entryCollateralAmount
    ...
    <Tippy
      content={
        <div className="min-w-max">
          {'decreasePnl' in position &&
          position.decreasePnl !== undefined &&
          position.decreasePnl !== 0 ? (
            <>
              <div className="flex justify-between">
                ...
                <span className="ml-4 inline-block">
                  <FormatNumber
                    nb={
                      showAfterFees
                        ? position.decreasePnl
                        : position.decreasePnl - decreaseFees
                    }
                    format="currency"
                    minimumFractionDigits={2}
          ...
          {'closePnl' in position &&
          position.closePnl !== undefined &&
          position.closePnl !== 0 ? (
            <div className="flex justify-between">
...
export const PnL = ({
  ...
  const fees = -(
    ('exitFees' in position ? position.exitFees : position.exitFeeUsd) +
    ('borrowFees' in position
      ? position.borrowFees
      : (position.borrowFeeUsd ?? 0))
  );
  const decreaseFees = -('totalDecreaseFees' in position
    ? position.totalDecreaseFees
    : 0);
  const closeFees = -('totalCloseFees' in position
    ? position.totalCloseFees
    : 0);
    ...
    <div className="flex flex-col items-center">
      ...
      {isHistory
        ? 'decreasePnl' in position &&
          position.decreasePnl !== undefined &&
          position.decreasePnl !== 0
          ? PnlTooltip(position, showAfterFees, fees, decreaseFees, closeFees)
          : PnlBlock(position, showAfterFees, fees)
        : PnlBlock(position, showAfterFees, fees)}
    </div>
  );
