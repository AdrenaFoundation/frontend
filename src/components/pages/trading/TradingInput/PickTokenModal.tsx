...
export function PickTokenModal({
  ...
  const [displayAllTokens, setDisplayAllTokens] =
    useState<boolean>(isDisplayAllTokens);
  ...
  const filteredTokenList = useMemo(
    () =>
      tokenList.filter((token) => {
        if (!displayAllTokens && !walletTokenBalances?.[token.symbol])
          return false;
        ...
        >
          ...
          {recommendedToken ? (
            <div className="flex gap-4 items-center mb-1 w-full">
              ...
              <div
                className="flex items-center cursor-pointer gap-2 rounded-lg pr-2 pt-1 pb-1"
                onClick={() => pick(recommendedToken)}
              >
                <Image
                  className="h-4 w-4"
            ...
            <div className="w-full flex gap-2">
              <Button
                ...
                className={twMerge(
                  'text-xs rounded-lg',
                  !displayAllTokens
                    ? 'border-white/30 text-white/80 border-2'
                    : 'border-transparent text-white/40 border bg-third',
                )}
                onClick={() => {
          ...
          <div
            className={twMerge(
              'flex flex-col w-full items-center mt-2 pr-2.5 max-h-full overflow-y-auto',
            )}
          >
            {filteredTokenList.length ? (
              filteredTokenList.map((token, i) => (
                ...
                >
                  ...
                  {walletTokenBalances?.[token.symbol] ? (
                    <div className="ml-auto pr-2 flex flex-col items-end justify-center">
              ...
              <div className="text-base mt-8 text-white/60">
                No tokens found
              </div>
            )}
          </div>
