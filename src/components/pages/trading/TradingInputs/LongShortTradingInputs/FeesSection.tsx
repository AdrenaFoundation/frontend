  ...
  const highSwapFeeTippyContent = (
    <div className="gap-4 flex flex-col">
      <div className="text-txtfade text-sm">
        The collateral you provided does not match the assets you&apos;r opening
        a position for, as such the platform will first have to do a Swap. Swap
        fees are dynamic and based on the Liquidity Pool&apos;s ratios, and
        currently that direction isn&apos;t favorable in term of fees. You can
        decide to go through or change the provided collateral.
      </div>
    </div>
  );
    ...
    <>
      <h5 className="hidden sm:flex items-center mt-2 mb-2">
        ...
        <span className="ml-1">
          <Tippy
            content={
              <p className="text-txtfade">
                0 BPS entry fees - 14 BPS exit fees
                {newPositionInfo && newPositionInfo.swapFeeUsd
                  ? ' - dynamic swap fees'
                  : ''}
                . 🎊 NO SIZE FEES! 🎊
              </p>
          ...
          >
            <Image src={infoIcon} width={14} height={14} alt="info icon" />
          </Tippy>
        </span>
      </h5>
      ...
      >
        ...
        >
          {newPositionInfo && !isInfoLoading ? (
            <AutoScalableDiv
              className=""
              bodyClassName="flex-col items-center justify-center mt-6"
            >
              {openedPosition && (
                <>
                  <TextExplainWrapper
              ...
              <TextExplainWrapper
                ...
                className="flex-col"
                onClick={() =>
                  setDisplayBorrowRateAsApr(!displayBorrowRateAsApr)
                }
              >
