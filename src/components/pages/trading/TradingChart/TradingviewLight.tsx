...
export default function TradingviewLight({
  token,
  positions,
}: {
  token: Token;
  ...
  useEffect(() => {
    function createWidget() {
      if (
        document.getElementById('chart-area-light') &&
        'TradingView' in window
      ) {
        const widget = new window.TradingView.widget({
          container: 'chart-area-light',
    ...
    if (!tvScriptLoadingPromise) {
      tvScriptLoadingPromise = new Promise((resolve) => {
        ...
        script.id = 'tradingview-widget-loading-scripts';
  ...
  return (
    <div className="flex flex-col w-full overflow-hidden">
      ...
      <div
        ...
        className={twMerge(
          'h-[200px] w-full flex flex-col',
          isLoading ? 'hidden' : '',
        )}
      >
        <div
          id="chart-area-light"
          className="h-full flex flex-col rounded-b-lg"
        />
      </div>
    </div>
  );
