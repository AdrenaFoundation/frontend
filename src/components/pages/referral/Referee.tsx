...
export default function Referee({
  ...
  const [newReferrerProfile, setNewReferrerProfile] = useState<
  ...
  const changeReferrer = useCallback(
    async (referrerProfile: PublicKey | null) => {
      const notification =
  ...
  return (
    <div
      className={twMerge(
        'flex flex-col gap-6 w-full pb-8 items-center',
      ...
      <div className="font-semibold w-full items-center flex justify-center flex-col gap-4 text-white/80 mt-4">
        {referrerProfile ? (
          <div className="flex gap-2">
            ...
            >
              {referrerProfile.nickname.length
                ? referrerProfile.nickname
                : referrerProfile.owner.toBase58()}

              {referrerProfile?.pubkey.toBase58() ===
              ADRENA_TEAM_PROFILE.toBase58() ? (
                <div className="text-xl">❤️</div>
              ) : null}
            </div>
          </div>
        ) : (
          'You are currently not supporting anyone'
        )}
        ...
        <div className="max-w-[25em] w-[90%] flex relative">
          <InputString
            ...
            className={twMerge(
              'mt-4 pt-[0.5em] pb-[0.5em] pl-4 pr-4 border border-gray-700 bg-transparent rounded-md placeholder:text-txtfade text-center w-full',
            )}
            inputFontSize="0.8em"
            onEnterKeyPressed={() => {
        ...
        <Button
          title={
            newReferrerNickname.length === 0
              ? 'Support'
              : newReferrerProfile === null
                ? 'Checking the referrer...'
                : newReferrerProfile === false
                  ? 'Cannot find referrer'
                  : `Support ${newReferrerProfile.nickname}`
          }
          variant="outline"
          disabled={newReferrerProfile === false || newReferrerProfile === null}
          onClick={() =>
            changeReferrer(
              newReferrerProfile ? newReferrerProfile.pubkey : null,
            )
          }
          className="w-60 mt-4"
        />
      ...
      referrerProfile.pubkey.toBase58() !== ADRENA_TEAM_PROFILE.toBase58() ? (
        <>
          ...
          <div className="text-center text-sm max-w-[60em] w-[100%] pl-4 pr-4">
            You can support the Adrena team through the referral system by
            choosing the team as your referrer. This is purely philanthropic.
            It&apos;s a way to show your support and contribute to the
            team&apos;s funding.
          </div>
