...
export default function PositionStatsCard({ symbol, stats, custodies, className, isLoading = false }: PositionStatsCardProps) {
    if (isLoading) {
        return (
            <div className={twMerge("p-4 border rounded-lg bg-[#050D14] flex-1 h-full flex items-center justify-center", className)}>
                <Loader />
            </div>
        );
    }
    ...
    return (
        <div className={twMerge("p-4 border rounded-lg bg-[#050D14] flex-1", className)}>
            ...
            <div className="space-y-4">
                {stats.map((stat) => (
                    <div key={stat.side} className="flex flex-col gap-3">
                        <h4 className={`text-sm font-medium ${stat.side === 'long' ? 'text-green' : 'text-redbright'}`}>
                        ...
                        {stats.length > 1 && <div className="h-[1px] bg-secondary/20 mt-2" />}
                    </div>
                ))}
            </div>
        </div>
