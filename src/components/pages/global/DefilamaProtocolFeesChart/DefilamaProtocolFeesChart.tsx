...
export default function DefilamaProtocolFeesChart({}: FeesChartProps) {
  ...
  const [mappedChartData, setMappedChartData] = useState<RechartsData[] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const mapped: RechartsData[] = defilamaFeesData
      .map(([timestamp, values]) => {
        ...
        const defaultValues = protocolsToWatch.reduce(
          (acc, protocol) => {
            acc[protocol] = 0;
            return acc;
          },
        ...
        const v = Object.entries(values).reduce(
          (acc, [key, value]) => {
  ...
  useEffect(() => {
    ...
    const dataPeriod =
      period === '7d' ? 7 : period === '1M' ? 30 : period === '3M' ? 90 : 180;

    setChartData(mappedChartData?.slice(-dataPeriod));
      ...
      title={
        <div className="flex flex-col">
          ...
          <div className="text-xxs lowercase opacity-30 cursor-pointer hover:opacity-60">
            Source:{' '}
            <a
              href="https://defillama.com/fees/chain/solana?category=Derivatives"
              target="_blank"
            >
