      ...
      allStakingStats.byRemainingTime[stakingType]
        ...
        .reduce(
          (acc, { endTime, tokenAmount }) => {
            ...
            if (periodMode === '7D' && endTime > now + 7 * 24 * 60 * 60)
              return acc;
            if (periodMode === '1M' && endTime > now + 30 * 24 * 60 * 60)
              return acc;
            if (periodMode === '3M' && endTime > now + 90 * 24 * 60 * 60)
              return acc;
            if (periodMode === '6M' && endTime > now + 180 * 24 * 60 * 60)
              return acc;
    ...
    <div className="flex flex-col w-full h-full">
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar
            dataKey={infoMode}
            fill={stakingType === 'ALP' ? '#256281' : '#a82e2e'}
          />
        </BarChart>
      </ResponsiveContainer>
    </div>
