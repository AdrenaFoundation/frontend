...
const colors = [
  '#c6bf4e', '#e9de49', '#6aa87e', '#aa70e4', '#3e5fe4',
  '#7ace6f', '#6d68c0', '#8ec2e4', '#b091d6', '#debc6b',
  '#73c464', '#4358d5', '#a88cdc', '#b5de5b', '#71ce94',
  '#a467b8', '#799f94', '#9aa267', '#8cdbc2', '#c6a569',
  '#ecb77c', '#98d4cc', '#95e5e3', '#91b3f0', '#4764df',
  '#cf6dea', '#88a382', '#81ab7a', '#7065cf', '#e0ce58',
  '#68d7c8', '#79d4e0', '#d4a95b', '#6870bb', '#dea76c',
  '#6748cb', '#c688df', '#5e54dc', '#d187ca', '#87b398',
  '#68cd9f', '#63bfe1', '#c1b971', '#5d6fd7', '#dec67d',
  '#67aa86', '#c982bd', '#97d7ec', '#92a66c', '#5d43b5',
  '#b992e7', '#6b4ed0', '#576ab7', '#bdc15e', '#414bb9',
  '#dfc26b', '#3a7ad0', '#ce79b7', '#9ea471', '#6fac91'
];
...
export default function UsersCohortsChart() {
  ...
  useEffect(() => {
    ...
    const timestamps = Array.from(cohorts.reduce((set, cohort) => {
      cohort.activity_week.forEach((time) => {
        set.add(new Date(time).toLocaleString('en-US', {
          day: 'numeric',
          month: 'numeric',
        }));
      });
    ...
    const formatted: RechartsData[] = timestamps.map((time: string, i: number) => ({
      time,
      ...cohorts.reduce((acc, cohort) => {
        // Look for the week that matches the current timestamp
        const index = cohort.activity_week.findIndex((week) => week === cohorts[0].activity_week[i]);
    ...
    setCohortsInfo(cohorts.map((cohort, i) => ({
