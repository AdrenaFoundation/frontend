...
export default function TopProtocolsByFees() {
  ...
  const getTopProtocolsByFees = useCallback(async () => {
    ...
    try {
      ...
      const protocols = apiRetProtocols
        .filter((p) => typeof p[attributeName] != 'undefined')
        .map(
          (protocol) =>
            [protocol[attributeName], protocol.name, protocol.logo] as const,
        );

      // Sort protocols by total7d in descending order and take the 3 protocols UP and down Adrena
      const sortedProtocols = protocols.sort((a, b) => b[0] - a[0]);

      const adrenaIndex = sortedProtocols.findIndex(
        ([, protocolName]) => protocolName === 'Adrena Protocol',
      );

      // Extract the 3 protocols above and below Adrena
      const topProtocols = sortedProtocols
        .slice(Math.max(0, adrenaIndex - 3), adrenaIndex)
        .concat(sortedProtocols.slice(adrenaIndex, adrenaIndex + 4));

      setData(
        topProtocols.map(([fees, name, logo], i) => ({
          name,
          fees,
      ...
      <div className="flex flex-col h-full w-full max-h-[18em]">
        <div className="flex flex-col mb-3">
          ...
          <Tippy
            content={
              <div className="flex flex-col">
                <div>
                  Note: The ranking shown on DeFiLlama differs from the one
                  displayed here. We use the DeFiLlama API
                  <span className="text-white/50 text-sm ml-1">* </span>, while
                  their site uses a different data source for rankings.
                </div>

                <div className="text-white/50 text-sm mt-2">
                  *
                  https://api-docs.defillama.com/#tag/fees-and-revenue/get/overview/fees
                </div>
              </div>
            }
          >
            <div className="text-xxs lowercase opacity-30 cursor-pointer hover:opacity-60">
              Source:{' '}
              <a href="https://defillama.com/fees" target="_blank">
        ...
        <div className="flex flex-col">
          {data.map(({ name, fees, rank, logo }, i) => (
            <div
              key={name}
              className={`flex justify-between items-center border-b ${i === 0 ? 'border-t' : ''} border-bcolor p-1`}
            >
              <div className="flex items-center gap-2">
                <div
                  className={`text-sm font-semibold w-6 text-left ${name === 'Adrena Protocol' ? 'text-[#6ebeff]' : ''}`}
                >
                  {rank}
                </div>
                ...
                <img
                  src={logo}
                  alt={`${name} logo`}
                  className={`h-4 w-4 ${name === 'Adrena Protocol' ? 'grayscale' : 'grayscale'}`}
                  width={16}
                  height={16}
                />

                <div
                  className={`text-sm font-semibold ${name === 'Adrena Protocol' ? 'text-[#6ebeff]' : ''}`}
                >
                  {name}
                </div>
              </div>
              ...
              <div className={`text-sm`}>
                <FormatNumber
                  nb={fees}
                  format="currency"
                  className={`${name === 'Adrena Protocol' ? 'text-[#6ebeff]' : ''}`}
                />
              </div>
            </div>
          ))}
