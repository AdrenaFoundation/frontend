...
export default function VolumeBarChart({ isSmallScreen, yAxisBarScale }: VolumeChartProps) {
    ...
    try {
      ...
      const [historicalData, latestData, historicalCumulativeData] = await Promise.all([
        ...
        DataApiClient.getPoolInfo({
          ...
          queryParams: 'cumulative_trading_volume_usd=true&aum_usd=true&open_interest_long_usd=true&open_interest_short_usd=true',
          dataPeriod,
        }),
        ...
        DataApiClient.getPoolInfo({
          ...
          queryParams: 'cumulative_trading_volume_usd=true&aum_usd=true&open_interest_long_usd=true&open_interest_short_usd=true&sort=DESC&limit=1',
          dataPeriod: 1,
        }),
      ...
      const formattedData: RechartsData[] = timeStamp.slice(1).map(
        (time: string, i: number) => {
          ...
          return {
            ...
            'Utilization': ((open_interest_long_usd?.[i + 1] ?? 0) + (open_interest_short_usd?.[i + 1] ?? 0)) / (aum_usd?.[i + 1] || 1) * 100,
          };
        }
      );
      ...
      formattedData.push({
        ...
        'Utilization': ((latestData.open_interest_long_usd?.[0] ?? 0) + (latestData.open_interest_short_usd?.[0] ?? 0)) / (latestData.aum_usd?.[0] || 1) * 100,
      });
  ...
  return (
    <MixedBarLineChart
      title={'Daily Volume & AVG Utilization'}
      data={chartData}
      labels={[
        { name: 'Volume', color: '#cec161', type: 'bar', yAxisId: 'left', format: 'currency' },
        { name: 'Utilization', color: '#ffffff', type: 'line', yAxisId: 'right', format: 'percentage' },
      ]}
      period={period}
      setPeriod={setPeriod}
      periods={['1M', '3M', '6M', '1Y']}
