  ...
  return (
    ...
    >
      <rect
        ...
        style={{
          fill:
            selectedPosition && selectedPosition.equals(positionPubkey)
              ? 'blue'
              : color,
          stroke: '#fff',
...
export default function AllPositionsChart({
  ...
  const [data, setData] = useState<
    ...
    | null
  >(null);
  const [selectedPosition, setSelectedPosition] = useState<PublicKey | null>(
    null,
  );
  const [selectedPositionObject, setSelectedPositionObject] =
        ...
        .map((token) => {
          ...
          return {
            ...
            children: positions
              ...
              .map((position, j) => {
                const pnlPercentage =
                  position.pnl !== null && typeof position.pnl !== 'undefined'
                    ? (position.pnl * 100) / position.collateralUsd
                    : null;
                ...
                return {
                  ...
                  pnl:
                    position.pnl !== null
                      ? formatPriceInfo(position.pnl, 2)
                      : null,
                  pnlPercentage:
                    pnlPercentage !== null
                      ? formatPercentage(pnlPercentage, 2)
                      : null,
  ...
  useEffect(() => {
    ...
    setSelectedPositionObject(
      allPositions?.find((position) =>
        position.pubkey.equals(selectedPosition),
      ) ?? null,
    );
  ...
  if (allPositions !== null && !allPositions.length) {
    return (
      <div className="text-center w-full py-4 opacity-50 mt-auto mb-auto">
        No matches 📭
      </div>
    );
  }
    ...
    <div className="flex flex-col w-0 flex-1 h-full items-center">
      <div className="min-h-[9em] h-[20.3125rem] lg:h-[15.33rem] xl:h-[10.03625rem] w-full max-w-[60em] shrink-0 flex items-center justify-center pb-2 ">
        {selectedPositionObject !== null ? (
          ...
          <div className="w-full h-[80%] border-4 border-dashed border-bcolor flex text-xs items-center justify-center opacity-50">
            Click on a position to see the detail
          </div>
        )}
      </div>
        ...
        <div className="flex ml-4 gap-8">
          <div className="flex flex-col items-center justify-center text-xs gap-2 font-mono">
            ...
            <div
              ...
              style={{
                background:
                  'linear-gradient(to right, #d58f8f, #b54b4b, #721717, #3d0909, #75d775, #4cbf4c, #2c8c2c, #064406)',
              }}
            />
          </div>
          ...
          <div className="flex flex-col items-center justify-center text-xs gap-2 font-mono">
            ...
            <div
              ...
              style={{
                background:
                  'linear-gradient(to right, #f2b791, #e89b66, #bf5b00, #7a2e00, #b296e4, #9e78d1, #6e33b5, #4b0082)',
              }}
            />
          </div>
        </div>
