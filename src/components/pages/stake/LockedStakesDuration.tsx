...
export default function LockedStakesDuration({
  ...
  const decimals =
    lockedStakes[0].tokenSymbol === 'ADX'
      ...
      (acc, lockedStake) => {
        return {
          amountWithRewardMultiplier:
            acc.amountWithRewardMultiplier +
            nativeToUi(lockedStake.amountWithRewardMultiplier, decimals),
          amountWithLmRewardMultiplier:
            acc.amountWithLmRewardMultiplier +
            nativeToUi(lockedStake.amountWithLmRewardMultiplier, decimals),
        };
      },
      {
  ...
  const firstUnlock = lockedStakes.reduce(
    (first, lockedStake) =>
      first === null || first > lockedStake.endTime.toNumber()
        ? lockedStake.endTime.toNumber()
        : first,
    null as null | number,
  );
  ...
  const [timeRemainingFirstUnlock, setTimeRemainingFirstUnlock] = useState<
    number | null
  >(null);
  ...
  return (
    ...
    >
      <div
        className={twMerge(
          'flex flex-col w-full items-center bg-secondary',
          lockedStakes.length > 1 ? 'cursor-pointer' : '',
        )}
        onClick={() => {
      ...
      >
        <div
          className={twMerge(
            'border-b items-center justify-center bg-secondary w-full flex pl-4 pt-2 pb-2 pr-4',
          )}
        >
          <div className="flex gap-1">
            ...
            {lockedStakes.length > 1 ? (
              <div className="text-sm flex gap-1 ">
                <div className="hidden sm:block text-txtfade font-semibold">
                  |
                </div>
                <div className="flex">
                  <div className="sm:hidden text-txtfade font-semibold">
                    {'('}
                  </div>
                  <div className="text-txtfade font-semibold">
                    {lockedStakes.length}
                  </div>
          ...
          {firstUnlock !== null ? (
            <div className="flex items-center justify-center ml-auto flex-col sm:flex-row sm:gap-2">
              <div className="text-xs font-semibold text-txtfade">
                {lockedStakes.length > 1 ? 'First unlock in' : 'Unlocks in'}
              </div>
              <RemainingTimeToDate
                timestamp={firstUnlock}
                className="text-xs"
              />
            </div>
          ) : null}
        ...
        <div className="flex flex-col pt-2 pb-2 items-center relative w-full">
          ...
          {lockedStakes.length === 1 && lockedStakes[0].isGenesis ? (
            <div className="text-xxs bg-[#068862a0] font-semibold pt-[0.1em] pb-[0.1em] px-2 mt-1 rounded w-14 text-center">
              genesis
            </div>
          ) : null}

          <div
            className="sm:absolute right-2 bottom-2 text-xs text-txtfade underline opacity-40 hover:opacity-100 transition-opacity cursor-pointer p-1"
            onClick={(e) => {
              e.stopPropagation();
          ...
          <div className="flex border-t pt-2 pb-2 w-full justify-evenly flex-wrap gap-y-4 gap-x-4">
            <div className="flex items-center gap-2">
              <Image
                ...
                height={6}
                alt="Weight icon"
                className="mr-1 h-5 w-5 opacity-10"
              />
              ...
              <Tippy
                content={
                  <div className="text-sm w-60 flex flex-col">
                    <span>
                      USDC rewards are distributed based on the amount of{' '}
                      {token} staked and the duration of the lock using a weight
                      system.
                    </span>
                    <span className="mt-2 mb-2">Example:</span>
                    <span>
                      There are 1 USDC to redistribute and 2 {token} staked. 1{' '}
                      {token} is staked with x4 multiplier and 1 {token} is
                      staked with x1 multiplier.
                    </span>
                    <span className="mt-1">
                      Total Weight is 5. Reward rate is 0.2 USDC per weight. The
                      first {token} will get 0.8 USDC and the second {token}{' '}
                      will get 0.2 USDC.
                    </span>
                  </div>
                }
              ...
              >
                <div className="flex items-center flex-col">
                  <div className="text-xs text-txtfade font-semibold underline decoration-dotted">
                    USDC Reward Weight
                  </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Tippy
                content={
                  <div className="text-sm w-60 flex flex-col">
                    <span>
                      ADX rewards are distributed based on the amount of {token}{' '}
                      staked and the duration of the lock using a weight system.
                    </span>
                    <span className="mt-2 mb-2">Example:</span>
                    <span>
                      There are 1 USDC to redistribute and 2 {token} staked. 1{' '}
            ...
            {totalWeight.votingPower ? (
              <div className="flex items-center gap-2">
                <Image
                  ...
                  height={6}
                  alt="Voting icon"
                  className="mr-1 h-8 w-8 opacity-10"
                />

                <div className="flex items-center flex-col">
                  <div className="text-xs text-txtfade font-semibold">
                    Voting Power
                  </div>
                  <div className="flex items-center justify-center gap-1">
                    <FormatNumber
      ...
      {detailOpen ? (
        <div className="border-t-1">
          ...
          <Pagination
            ...
            totalPages={
              lockedStakes
                ? Math.ceil(lockedStakes.length / lockedStakesPerPage)
                : 0
            }
      ...
      {lockedStakes.length > 1 ? (
        <div className="w-full items-center justify-center flex flex-col border-t  cursor-pointer">
          <div
            className="flex items-center pb-2 justify-center w-full  pt-1"
            onClick={() => {
              setDetailOpen(!detailOpen);
            }}
          >
            <div className="flex gap-1 opacity-40 hover:opacity-100 cursor-pointer transition-opacity justify-center items-center">
              <div className="text-txtfade underline  text-xs">
                {!detailOpen ? 'See' : 'Hide'} all stakes
              </div>

              {lockedStakes.length > 1 ? (
                <div className="text-xs flex gap-1 font-semibold text-txtfade">
                  ({lockedStakes.length})
                </div>
              ) : null}
            </div>
          ...
          {!readonly ? (
            <div className="flex w-full">
              {!lockedStakes[0].isGenesis && (
                <Tippy
                  disabled={lockedStakes.some(
                    (lockedStake) =>
                      lockedStake.qualifiedForRewardsInResolvedRoundCount > 0,
                  )}
                  content={
                    <div className="flex flex-col justify-around items-center">
                      No locked stake eligible. To upgrade a locked stake, it
                      must have been locked for at least one round, generated
                      rewards, and had those rewards claimed. This process can
                      take up to 12 hours.
                    </div>
                  }
                  placement="auto"
                >
                  <div className="flex grow">
                    <Button
                      ...
                      title="Quick Upgrade"
                      disabled={lockedStakes.every(
                        (lockedStake) =>
                          lockedStake.qualifiedForRewardsInResolvedRoundCount ===
                          0,
                      )}
                      onClick={() => {
                        handleClickOnUpdateLockedStake(
                          lockedStakes.reduce((last, lockedStake) =>
                            last.endTime.toNumber() >
                            lockedStake.endTime.toNumber()
                              ? last
                              : lockedStake,
                          ),
        ...
        <div className="flex w-full">
          {timeRemainingFirstUnlock !== null &&
          timeRemainingFirstUnlock <= 0 ? (
            <Button
              variant="outline"
              size="xs"
              title="Redeem"
          ...
          (timeRemainingFirstUnlock === null ||
            ...
            <Tippy
              disabled={
                lockedStakes[0].qualifiedForRewardsInResolvedRoundCount !== 0
              }
              content={
                <div className="flex flex-col justify-around items-center">
                  To upgrade a locked stake, it must have been locked for at
                  least one round, generated rewards, and had those rewards
                  claimed. This process can take up to 12 hours.
                </div>
              }
              placement="auto"
            >
              <div className="flex grow">
                <Button
                  ...
                  size="xs"
                  disabled={
                    lockedStakes[0].qualifiedForRewardsInResolvedRoundCount ===
                    0
                  }
                  className="rounded-none py-2 w-20 border-b-0 border-l-0 border-r-0 text-txtfade border-bcolor bg-[#a8a8a810] grow h-8"
                  title="Upgrade"
