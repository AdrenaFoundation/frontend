...
export default function LockedStake({
  ...
  return (
    <div
      key={lockedStake.id.toString()}
      className={twMerge('flex flex-col p-2 w-full', className)}
    >
      <div
        className={twMerge(
          'flex w-full items-center gap-4 pb-4 sm:pl-2 sm:pr-2 flex-col sm:flex-row',
          !readonly ? 'sm:pb-2' : 'sm:pb-0',
        )}
      >
        <div className="font-semibold text-xs border w-full sm:rounded-md sm:w-16 h-6 items-center justify-center bg-[#1e272e] flex text-txtfade">
          Stake #{lockedStake.index}
        </div>
        ...
        <div className="ml-0 flex gap-0 items-center">
          ...
          {lockedStake.isGenesis ? (
            <div className="text-xxs bg-[#068862a0] font-semibold pt-[0.1em] pb-[0.1em] px-2 ml-4 rounded w-14 text-center">
              genesis
            </div>
          ) : null}
        </div>
        ...
        <div className="flex gap-2 items-center sm:ml-auto">
          ...
          <RemainingTimeToDate
            timestamp={lockedStake.endTime.toNumber()}
            className="text-xs"
          />
        </div>
      </div>
