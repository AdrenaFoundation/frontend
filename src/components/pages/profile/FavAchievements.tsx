...
export default function FavAchievements({ userProfile, favoriteAchievements, setIsUpdatingMetadata, setActiveUpdateTab }: {
  ...
  const achievements = ACHIEVEMENTS.filter(
    (a) => favoriteAchievements?.includes(a.index),
  );
    ...
    <>
      <div>
        <div className='flex flex-row justify-between items-center px-4 pt-2'>
          <p className='font-boldy text-lg mb-3'>Achievements <span className='opacity-50 ml-1'>{ACHIEVEMENTS.filter(
            (achievement) => userProfile.achievements?.[achievement.index] > 0,
          ).length} / {ACHIEVEMENTS.length}</span></p>
          <p className='text-xs opacity-50 hover:opacity-100 transition-opacity duration-300 cursor-pointer' onClick={() => {
            setIsUpdatingMetadata(true)
            setActiveUpdateTab('achievements')
          }}>Edit</p>
        </div>
        <div className='relative flex flex-row justify-center items-center gap-6 px-4 overflow-hidden cursor-pointer' onClick={() => setIsModalOpen(true)}>
