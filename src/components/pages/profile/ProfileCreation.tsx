...
export default function ProfileCreation({
  ...
  const [trimmedNickname, setTrimmedNickname] = useState<string>(nickname ?? '');
  ...
  useEffect(() => {
    if (trimmedNickname.length < 3 || trimmedNickname.length > 24 || !window.adrena.client.readonlyConnection) {
      return;
    }

    const userNicknamePda = window.adrena.client.getUserNicknamePda(trimmedNickname);
      ...
      <div className="flex flex-col w-full z-20 p-7 self-center rounded-md items-center">
        ...
        <div className="flex flex-col mt-4">
          <h5 className="flex flex-col text-sm mb-3 opacity-50 items-center text-center">
            Your trader profile is required to trade. It tracks your PnL, fees, and trading stats, giving you a detailed view of your performance. It also serves as your Adrena identity in the ecosystem.
          </h5>

          <div className='w-full h-[1px] bg-bcolor mt-6 mb-6' />

          <h5 className="flex flex-col mb-3 self-center">Choose your Nickname!</h5>

          <h5 className="flex flex-col mb-3 self-center text-white/50">Careful! Once your nickname is set, changing it will cost ADX. Choose wisely!</h5>
          ...
          <div className="flex flex-col items-center pb-4">
            ...
            <div className='h-[2em]'>
              {(trimmedNickname && trimmedNickname.length < 3) || !trimmedNickname ?
                <div className="text-red-500 text-xs text-center mb-4 text-txtfade font-semibold">Nickname must be at least 3 characters</div> :
                null}
              ...
              {trimmedNickname && alreadyTakenNicknames[trimmedNickname] === true ?
                <div className="text-red-500 text-xs text-center mb-4 text-yellow-400 font-semibold">Nickname is already taken</div> :
                null}

              {trimmedNickname && alreadyTakenNicknames[trimmedNickname] === false ?
                <div className="text-red-500 text-xs text-center mb-4 text-green font-semibold">Nickname is available</div> :
                null}
            </div>
            ...
            <Button
              disabled={
                nickname
                  ? !(trimmedNickname.length >= 3 && trimmedNickname.length <= 24)
                  : true
              }
