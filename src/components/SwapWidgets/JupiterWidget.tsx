...
export default function JupiterWidget({
  ...
  const stableSignTransaction = useCallback(
    async (transaction: Transaction) => {
      try {
        const signedTx = await (
          connectedAdapter as unknown as {
  ...
  const stableSignAllTransactions = useCallback(
    ...
    if (connectedAdapter && appWalletState) {
      return {
        ...
        sendTransaction: async (
          ...
          try {
            const result = await connectedAdapter.sendTransaction?.(
              transaction,
              connection,
            );
            return result;
