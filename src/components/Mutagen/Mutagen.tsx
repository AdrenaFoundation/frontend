  ...
  <div className="flex flex-col mb-3 items-center">
    ...
    <div className="w-full mt-4 space-y-3">
      {mutagenSources.map((source, index) => (
        <div
          key={index}
          className="bg-[#0f1114] p-4 rounded-lg border border-[#1f2124] hover:border-[#2f3134] transition-colors"
        >
...
const contentIfMutagens = (userMutagens: EnrichedUserMutagens) => {
  ...
  return (
    <div className="flex flex-col mb-3 items-center">
      <p className="text-txtfade text-sm mt-2 text-center">
        <span className="font-boldy">Mutagen</span> is an elusive resource
      ...
      <div className="w-full mt-4 flex flex-col gap-4">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h4 className="text-white/80 tracking-wider font-boldy uppercase text-xs">
              All seasons
            </h4>
            <p className="gap-1 flex items-center">
              <span className="font-boldy text-white/80 text-xxs">Earned</span>

              <span className="font-mono text-xs text-white/80">
                {formatNumber(userMutagens.totalTotalPoints, 2, 2)}
              </span>

              <span className="font-boldy text-white/80 text-xxs">mutagen</span>
            </p>
          </div>
          ...
          <div className="flex justify-between flex-wrap text-xs">
            {stats.map((stat, index) => (
              <div key={index} className="flex items-center gap-1">
                ...
                <span
                  className="text-[1.1em] font-boldy"
      ...
      <div className="text-xs uppercase font-boldy tracking-wider text-white/80 mt-2">
