...
export default function QuestComp({
  ...
  return (
    <div className={twMerge('flex gap-1 justify-between', className)}>
      <div className="flex justify-between items-center">
        <div className="flex flex-col">
          ...
          {quest.description && (
            <span className="text-white/60 text-xs">{quest.description}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col items-end justify-center gap-2 text-white/50 font-mono min-w-[8em]">
        {[1, 2, 3, 4, 5, 6].map((index) => {
          const targetType =
            quest[
              `targetType${index === 1 ? '' : index}` as keyof EnrichedSeasonQuestProgress
            ];
          const isCondition =
            quest[
              `isCondition${index === 1 ? '' : index}` as keyof EnrichedSeasonQuestProgress
            ];
          const currentValue =
            quest[
              `currentValue${index === 1 ? '' : index}` as keyof EnrichedSeasonQuestProgress
            ];
          const targetValue =
            quest[
              `targetValue${index === 1 ? '' : index}` as keyof EnrichedSeasonQuestProgress
            ];
          ...
          if (
            ...
            const clampedProgress = Math.min(
              ...
              Math.max(
                0,
                ((currentValue as number) / (targetValue as number)) * 100,
              ),
            );
            ...
            return (
              ...
              >
                <>
                  ...
                  <div
                    className={twMerge(
                      'w-[4em] bg-gray-500/30 rounded-full h-2 mt-1',
                    )}
                  >
                    <div
