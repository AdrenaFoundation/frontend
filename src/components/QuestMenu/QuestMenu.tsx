...
export default function QuestMenu({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (userSeasonProgress) {
        if (
          Date.now() >= new Date(userSeasonProgress.startDate).getTime() &&
          Date.now() <= new Date(userSeasonProgress.endDate).getTime()
        ) {
          setInSeason(true);
  ...
  const [nextSaturdayUTC, setNextSaturdayUTC] = useState(
    getNextSaturdayUTC().getTime() / 1000,
  );
  ...
  const classNameTitle =
    'mt-2 mb-2 animate-text-shimmer bg-clip-text text-transparent bg-[length:300%_100%] bg-[linear-gradient(110deg,#FA6724,45%,#FAD524,55%,#FA6724)] ';
  ...
  const body = (
    <>
      <div className="relative flex flex-row items-center w-full">
        ...
        <div
          className="absolute -bottom-2 w-full h-[8em] bg-gradient-to-t from-[#07131D] to-transparent cursor-pointer"
          onClick={() => {
            setIsOpen(!isOpen);
          }}
        />
      ...
      <div
        className={twMerge(
          'relative border-t border-white/10 overflow-y-auto w-full',
          inSeason ? '' : 'blur-md',
        )}
      >
        <div className="flex flex-col p-4">
            ...
            <>
              <div className="flex w-full justify-between">
                ...
                <div className="flex gap-2 opacity-30 mt-1">
                  <div className="text-xxs font-mono">reset in</div>
          ...
          {userSeasonProgress.mutations.length > 0 && (
            <div className="mt-2 flex flex-col">
              <h3 className={classNameTitle}>Mutations</h3>
          ...
          {userSeasonProgress.quests.weeklyQuests.length > 0 && (
            <>
              <div className="flex w-full justify-between items-center">
                <h3 className={classNameTitle}>Weekly Quests</h3>
          ...
          <>
            <div className="flex w-full justify-between">
              <h3 className={classNameTitle}>Streaks</h3>
      ...
      className={twMerge(
        ...
        isMobile
          ? window.location.pathname === '/trade'
            ? 'bottom-[calc(7.7rem+env(safe-area-inset-bottom))] sm:bottom-[calc(4.3rem+env(safe-area-inset-bottom))] left-4'
            : 'bottom-[calc(4.3rem+env(safe-area-inset-bottom))] left-4'
          : 'bottom-0 left-0',
        className,
      )}
    >
      <AnimatePresence>
        {!isOpen && (
          <motion.div
            key="closed-state"
            className={twMerge(
              '-translate-x-6 translate-y-6 cursor-pointer',
              !isMobile &&
                'hover:translate-x-0 hover:translate-y-0 transition-transform duration-300',
            )}
            onClick={() => {
              setIsOpen(!isOpen);
            }}
