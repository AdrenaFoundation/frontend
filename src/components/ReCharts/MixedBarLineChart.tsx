...
export default function MixedBarLineChart<T extends string>({
  ...
  const lineLabels = labels.filter(
    (label) =>
      label.type === 'line' && !label.name.toLowerCase().includes('cumulative'),
  );
  ...
  const maxCumulativeValue = data.reduce((max, point) => {
    const cumulativeValue = cumulativeLabel
      ? Number(point[cumulativeLabel.name] || 0)
      : 0;
        ...
        <ComposedChart data={data}>
          ...
          <Tooltip
            content={
              <CustomRechartsToolTip
                ...
                gmt={gmt}
                events={events}
                lineDataKeys={lineLabels
                  .map((label) => label.name)
                  .concat(cumulativeLabel ? [cumulativeLabel.name] : [])}
                precision={0}
              />
            }
            cursor={false}
          ...
          {cumulativeLabel && (
            <Line
              ...
              type="monotone"
              dataKey={
                hiddenLabels.includes(cumulativeLabel.name)
                  ? cumulativeLabel.name + ' '
                  : cumulativeLabel.name
              }
