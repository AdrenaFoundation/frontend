...
export default function MoreMenu({
  ...
  return (
    <div className="relative" ref={ref} onMouseLeave={handleMouseLeave}>
      <h5
        className={twMerge("whitespace-nowrap font-medium opacity-50 hover:opacity-100 cursor-pointer",
          (PAGES.some((page) => pathname === page.link) || isOpen) ? 'grayscale-0 opacity-100' : 'grayscale'
        )}
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col gap-3 flex-1">
              {PAGES.map((page) => {
                return (
                  <Link
                    className={twMerge("flex flex-row items-center justify-between group border border-bcolor bg-transparent hover:bg-bcolor/20 p-2 pr-4 rounded-xl transition duration-300",
                      pathname === page.link ? 'bg-bcolor/20 border-white/10' : ''
                    )}
                    href={page.link}
                    key={page.name}
                ...
                {EXTERNAL_LINKS.map((page) => {
                  return (
                    ...
                    >
                      ...
                      <Image src={arrow} alt="arrow icon" className="w-2 h-2" />
                    </Link>
                  );
                })}
              </div>
