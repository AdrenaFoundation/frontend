    ...
    return (
        <div className={twMerge(
            ...
            <div className="flex items-center gap-3 min-w-0">
                ...
                <div className={`size-8 rounded-full flex items-center justify-center flex-shrink-0 ${token.icon ? 'hidden' : ''}`}>
                    <span className="rounded-full size-full bg-gray-900 border border-bcolor">
                    </span>
                </div>

                <div className="min-w-0 flex-1">
                    <div className="text-white text-sm leading-tight truncate">{token.name === 'Wrapped SOL' ? 'Solana' : token.name}</div>
                ...
                <div className="text-xs leading-tight">
                    {token.priceChange24h !== undefined ? (
                        <FormatNumber
                            ...
                            className={twMerge('text-mono text-xs', token.priceChange24h >= 0 ? 'text-green' : 'text-red')}
                        />
                    ) : (
