...
export function SendTokenView({
  ...
  tokenBalancesWithPrices: Array<
    TokenBalance & { priceUsd?: number; valueUsd?: number }
  >;
  ...
  const [selectedToken, setSelectedToken] = useState<
    (typeof tokenBalancesWithPrices)[0] | null
  >(
    tokenBalancesWithPrices.find((t) => t.symbol === 'SOL') ||
      tokenBalancesWithPrices[0] ||
      null,
  );

  const [recipientAddress, setRecipientAddress] = useState<string | null>('');
  const [recipientAddressPubkey, setRecipientAddressPubkey] =
  ...
  const { getProfilePicture, getDisplayName, isLoadingProfiles } =
  ...
  useEffect(() => {
    ...
    const currentWallet = enhancedWallets.find(
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (amountNumber > selectedToken.uiAmount) {
        setError(
          `Insufficient balance. Available: ${selectedToken.uiAmount} ${selectedToken.symbol}`,
        );
        return;
      }
    ...
    <div className="flex flex-col grow gap-4">
      ...
      <div className="space-y-2 relative" ref={dropdownRef}>
        ...
        <div className="relative">
          ...
          />
          {enhancedWallets &&
            enhancedWallets.filter((w) => w.address !== walletAddress).length >
              0 && (
              <button
              ...
              >
                {enhancedWallets
                  ...
                  .map((enhancedWallet) => {
                    ...
                    const isSelected =
                      enhancedWallet.address === recipientAddress;
                    return (
                      <div
                      ...
                      >
                        <button
                          onClick={() => {
                            ...
                            setRecipientAddressPubkey(
                              new PublicKey(enhancedWallet.address),
                            );
                            setRecipientAddressError(null);
                            setShowWalletDropdown(false);
                          }}
        ...
        <AnimatePresence mode="wait">
          {recipientAddressPubkey &&
            !recipientAddressError &&
            (() => {
              const knownWallet = enhancedWallets?.find(
                (w) => w.address === recipientAddress,
              );
              if (knownWallet) {
                const walletData = getWalletDisplayDataForEnhancedWallet(
                  knownWallet,
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium text-txtfade">
                      Unknown wallet
                    </span>
                  </div>
                  <div className="text-xs text-txtfade/70 leading-relaxed">
                    Please verify this is a valid Solana address by checking the
                    account exists on the explorer before sending.
                  </div>
                </motion.div>
              );
            })()}
