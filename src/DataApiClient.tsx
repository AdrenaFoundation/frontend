...
export default class DataApiClient {
  ...
  public static async getLastPrice(): Promise<{
    ...
    try {
      const result = await fetch(`${DataApiClient.DATAPI_URL}/last-price`).then(
        (res) => res.json(),
      );
  ...
  public static async getChartAprsInfo(
    nbDays: number,
    stakingType?: 'lm' | 'lp',
    lockPeriod?: number,
  ): Promise<{
  ...
  public static async getFactionsLeaderboards(): Promise<FactionsLeaderboardsData | null> {
    try {
      ...
      return {
        weekly: {
          weekDatesStart: result.data.weekly.week_dates_start.map(
            (x) => new Date(x),
          ),
          weekDatesEnd: result.data.weekly.week_dates_end.map(
            (x) => new Date(x),
          ),
          ...
          bonkLeaderboard: result.data.weekly.bonk_leaderboard.map((x) =>
            x.map(
              ({
                user_wallet,
                team,
          ...
          jitoLeaderboard: result.data.weekly.jito_leaderboard.map((x) =>
            x.map(
              ({
                user_wallet,
                team,
  ...
  public static async getSeasonLeaderboards({
    ...
    try {
      ...
      return {
        startDate: new Date(result.data.start_date),
        endDate: new Date(result.data.end_date),
        weekLeaderboard: result.data.week_leaderboard.leaderboard.map(
          (leaderboard, i) => ({
            startDate: new Date(
              result.data.week_leaderboard.week_dates_start[i],
            ),
  ...
  >({
    ...
    try {
      ...
      const rankedRewards: RankedRewards[] = result.data.ranked_divisions.map(
        ...
        ...(showAchievements && {
          achievements: {
            biggestLiquidation: {
              ...
              rewardToken:
                result.data.achievements.biggest_liquidation.reward_token,
            },
            feesTickets: {
            ...
            jitosolTickets: {
              ...
              rewardToken:
                result.data.achievements.jitosol_tickets.reward_token,
            },
          },
        }),
        ...(showTraderDivisions && {
  ...
  public static async getPositions({
    ...
    try {
      const response = await fetch(
        `${DataApiClient.DATAPI_URL}/v3/position?user_wallet=${
          walletAddress
      ...
      if (
        ...
        (apiData && apiData.positions && apiData.positions.length === 0)
      )
        return null;
  ...
  public static async getTraderInfo({
    ...
    try {
      ...
      if (typeof apiData === 'undefined' || !apiData) return null;
      ...
      return {
        ...
        totalNumberPositionsLiquidated:
          apiData.total_number_positions_liquidated,
  ...
  public static async getPoolInfo({
    ...
    try {
      ...
      const url = `${DataApiClient.DATAPI_URL}/${dataEndpoint}?${queryParams}&start_date=${startDate.toISOString()}&end_date=${new Date().toISOString()}`;
  ...
  public static async getCustodyInfo(
    ...
    try {
      ...
      if (dataPeriod) {
        startDate.setDate(startDate.getDate() - dataPeriod);
        url = `${DataApiClient.DATAPI_URL}/${dataEndpoint}?${queryParams}&start_date=${startDate.toISOString()}&end_date=${new Date().toISOString()}`;
      } else {
        url = `${DataApiClient.DATAPI_URL}/${dataEndpoint}?${queryParams}`;
      }
  ...
  public static async fetchClaimsHistory({
    ...
    if (offset > 0 && totalClaimsCount === 0) {
      console.log(
        `DataApiClient: No claims found at offset ${offset}. You may have reached the end of available data.`,
      );
    }
  ...
  public static async getPositionTransactions({
    ...
    try {
      ...
      const transactions: PositionTransaction[] = result.data.map(
        (item: RawTransactionPositionData) => ({
