...
test.describe('Middleware Security', () => {
  ...
  ALLOWED_ORIGINS.forEach((origin) => {
    test(`allows API requests from allowed origin: ${origin}`, async ({ request }) => {
      ...
      if (isProductionTest) {
        ...
        expect(response.headers()['access-control-allow-methods']).toBe(EXPECTED_CORS_HEADERS['access-control-allow-methods']);
        expect(response.headers()['access-control-allow-headers']).toBe(EXPECTED_CORS_HEADERS['access-control-allow-headers']);
        expect(response.headers()['access-control-allow-credentials']).toBe(EXPECTED_CORS_HEADERS['access-control-allow-credentials']);
      } else {
  ...
  test('validates security blocking logic implementation', async ({ request }) => {
    ...
    if (isProductionTest) {
      ...
      expect(allowedResponse.headers()['access-control-allow-origin']).toBe(ALLOWED_ORIGINS[0]);
    } else {
      // In development, same behavior
  ...
  test('handles same-origin requests without Origin header', async ({ request }) => {
    const isProductionTest = process.env.TEST_ENV === 'production';
  ...
  test('handles OPTIONS preflight requests with proper CORS headers', async ({ request }) => {
    const isProductionTest = process.env.TEST_ENV === 'production';
  ...
  test('handles malformed origin headers gracefully', async ({ request }) => {
    ...
    for (const malformedOrigin of malformedOrigins) {
      ...
      console.log('handles malformed origin ', response.status())

      // Should not crash the middleware, should still return a response
